...
- 登録済みの代表フォームに加え、指板上から生成した複数のコンパクトフォームを選択できる
- `major` `minor` `5` `sus2` `sus4` `dim` `aug` `6` `m6` `7` `maj7` `m7` `m7b5` `dim7` `add9` `maj9` `m9` `7sus4` `madd9` `mMaj7` `6/9` `9` `11` `13` `m11` `7b5` `7#5` `7b9` `7#9` を選択できる
- コードビルダーモーダルで押弦を直接編集し、開始フレットと表示フレット数を手動調整できる
- 表示範囲の自動調整、候補コード名、ベース音、構成音、ユニーク音、各弦の音を確認できる
- 完全一致に加えて許可された省略音を含むフォームを判定し、ルート省略などの推測候補は「高度な候補」として分離表示できる
